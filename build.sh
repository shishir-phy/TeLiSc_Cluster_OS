...
iso_name=TeLiSc_Cluster
iso_label="TeLiSc_Cluster_$(date +%Y%m)"
...
make_setup_mkinitcpio() {
    ...
    for _hook in archiso archiso_shutdown archiso_pxe_common archiso_loop_mnt archiso_pxe_nbd archiso_pxe_http archiso_pxe_nfs; do
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "${iso_name}-${iso_version}-X86_64(beta).iso"
}
